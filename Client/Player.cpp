...
Player::Player(b2World& physic_world, std::string name) : Actor(physic_world), m_cadence(5), m_timer_fire(1 /m_cadence), m_timer_stun(1.0f)
{
	m_moveSpeed = 15;
	m_isShooting = false;
	m_isLoading = false;
...
void Player::getMovement(sf::RenderWindow &window)
{
	...
	b2Vec2 act_velocity = body->GetLinearVelocity();
...
void Player::extractDataEnnemyPlayer(sf::Packet packet)
{
	...
	while (!packet.endOfPacket())
	{
		...
		bullets.push_back(std::make_unique<Arrow>(b2Vec2(position_bullet.x , position_bullet.y), mouseDirection, m_physic_world, FRIENDLY_ARROW, ENEMY_ARROW | ENEMY_PLAYER, *this));
	}
}